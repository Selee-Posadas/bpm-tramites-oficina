...
| SEC-07 | Usuario externo solo puede ver y operar en trámites donde participe | pág. 4, 11, 14 | `src/modules/auth/infrastructure/guards/tramite-ownership.guard.ts` y `ExternoLayout` | Unit: `auth-guards.spec.ts` | VERIFIED | Bloqueo estricto de rutas a usuarios no autenticados y 403 en acceso a trámites ajenos |
| SEC-08 | Operador interno solo puede ver trámites asignados a su área | pág. 4 | `src/modules/tramites/application/use-cases/listar-tramites.use-case.ts` y `obtener-tramite.use-case.ts` | Unit: `test/unit/use-cases.spec.ts` | VERIFIED | Restricción estricta de área en listado y detalle con 403 y tests automatizados |
